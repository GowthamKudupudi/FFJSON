                        ...
                        comment += objId;
							...
							switch (objId[0]) {
                              ...
                              string sLink = lnr.m_sLink +
                                 (*fmMapSequence.m_pvpsMapSequence)[size-2]->
                                 first;
                              FFJSON* pOverLooker=
                                 MarkAsUpdatable(sLink,
                                                 *(lnr.m_pRef?lnr.m_pRef:this));
                              FeaturedMember fm;
...
string FFJSON::prettyString (
   ...
   FFJSONPrettyPrintPObj * pObj, bool printFilePath
) const {
	string ps;
		...
		case OBJ_TYPE::ARRAY:
		{
			...
			while (i < objarr.size()) {
				...
				if (t != UNDEFINED && t != NUL) {
                  ...
                  json, printComments, indent + 1,&lfpo,printFilePath);
					ps.append(sMem);
				} else if (t == NUL) {
					ps.append(indent + 1, '\t');
				}
...
FFJSON& FFJSON::addLink (FFJSON* obj, string label) {
   vector<string>* prop = new vector<string>();
   explode(".", label, *prop);
   if (obj) {
      freeObj(true);
